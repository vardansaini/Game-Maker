...
{
	public class GridPlacementPrev : Lockable
	{
		[HideInInspector]
		public SpriteData CurrentSprite; // Initialized by the sprite menu

		[SerializeField]
		private GridObjectPrev previewObject;
		[SerializeField]
		...
		private void Update()
		{
			...
			if (!IsLocked)
			{
				...
				for (float i = 0.25f; i <= 1; i += 0.25f)
				{
					...
					if (mode == PlacementMode.Level)
					{
						...
						if (Input.GetMouseButton(1))
						{
							...
							{
								if (GridPrev.Instance.ContainsGridObject(true, mouseX, mouseY))
									deletionLayer = true;
								else if (GridPrev.Instance.ContainsGridObject(false, mouseX, mouseY))
									deletionLayer = false;
							}

							// Remove existing grid object based on deletion layer
							if (deletionLayer != null)
								if (GridPrev.Instance.ContainsGridObject(deletionLayer.Value, mouseX, mouseY))
									GridPrev.Instance.RemoveGridObject(deletionLayer.Value, mouseX, mouseY);
						}
						else if (Input.GetMouseButton(0) && CurrentSprite.HoldToPlace && GridPrev.Instance.CanAddGridObject(CurrentSprite, spriteX, spriteY))
						{
							// Place new grid object (if hold-to-place)
							GridPrev.Instance.AddGridObject(CurrentSprite, spriteX, spriteY, true);
						}
				...
				if (mode == PlacementMode.Level)
				{
					...
					if (Input.GetMouseButtonDown(0) && !Input.GetMouseButton(1) && GridPrev.Instance.CanAddGridObject(CurrentSprite, spriteX, spriteY))
						GridPrev.Instance.AddGridObject(CurrentSprite, spriteX, spriteY, true);
				...
				previewObject.gameObject.SetActive(GridPrev.Instance.CanAddGridObject(CurrentSprite, spriteX, spriteY));
			}
