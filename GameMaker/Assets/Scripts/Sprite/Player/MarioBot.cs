...
public class MarioBot : BaseSprite
{
	//Max run speed
	...
	public enum Key { KEY_LEFT = 0, KEY_RIGHT = 1, KEY_JUMP = 2 }
	public bool[] keyEmulated = { false, false, false };
	public int nodeToChaseIndex = 0;
	public bool lookedAtNode = false; // Turns true once looked at node
	...
	void Update()
	{
		...
		// Reset AI Input
		keyEmulated[(int)Key.KEY_JUMP] = false;
		keyEmulated[(int)Key.KEY_RIGHT] = false;
		keyEmulated[(int)Key.KEY_LEFT] = false;
	}

	// Sets path from Map object. Agent will run it down
	public void SetPath()
	{

	}
	// Emulates keys based on node to chase
	public void HandleInput()
	{
		...
		if (horizontalSignDif == 1)
		{
			keyEmulated[(int)Key.KEY_RIGHT] = true;
		}
		else if (horizontalSignDif == -1)
		{
			keyEmulated[(int)Key.KEY_LEFT] = true;
		}
